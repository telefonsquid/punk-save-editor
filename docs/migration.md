...
| `UNKNOWN effect types (add to EFFECT_KINDS)` | extract-module-effects.ts | A new `ModuleEffect` subclass. All known ones share a shape (one FloatSeries magnitude + usually a Resource + flat scalars); decompile the new class, add one row to `EFFECT_KINDS`, and register its `kind` in `src/lib/game/effect-kinds.ts` — `bun run check` then points at the stat line it still needs in `module-stats.ts`. |
| `unknown effect kind` (ERROR) | check-data | `module-effects.json` holds a kind the code doesn't know — the JSON was generated by a different code state. Re-run `bun run extract` on the current checkout. |
...
| `Sprites UI GridSlotTypes not found` / `is NxN, expected 64 square` | extract-grid-icons | The grid backdrop's shader sheet was renamed or re-laid-out. Re-read `Unlit/GridBackground` and `ModuleGridWidget.RefreshBackgroundTexture` — the quadrant boxes in `SLOT_QUADRANTS` are the LUT encoding, not a layout choice. |
| `sprite HUD_ModuleIcons_N not found` | extract-grid-icons | An art pass renumbered the HUD atlas. Find the notch/chevron sprite again (it is on the `ConnectionWidget` / `ModuleIcon_UpgradeUnit` prefab) and fix the constant. |
| `grid-icons.json has no frame for module type X` (ERROR) | check-data | A new `ModuleType` shipped without a `background`, or the extractor could not read it. The grid editor falls back to the rect frame, so this is cosmetic, but the new category has no art of its own. |
