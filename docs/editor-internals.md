...
src/lib/save/          the save files (nothing here knows about the UI)
  ...
  grid.ts      the module grid in `entities`: gridOwners (every entity carrying
               one — enemies too, so callers filter), snapshotGrid -> the plain
               view the simulation runs on
  ship.ts      shipResources + the caps/regen walk, exact via the grid simulation
...
src/lib/game/          static game knowledge (nothing here touches a save)
  data.ts      assets/names/categories, module info + effects + series math, equippableModules,
               slotTypeInfo (what a grid cell's type does)
  grid-rules.ts the module-grid simulation, ported from the game: clusters, power,
               level deltas, the five validation checks, randomizeSlots
               (docs/grid-editor.md)
...
src/lib/components/    the primitives — restyle THESE, not every panel. Section / Dialog /
                       ...
                       Select / CloseBadge / ConnectionToggles, plus the module pieces
                       both module surfaces share (ModuleStatLine, ModuleGroupHeading).
                       See docs/design.md.
  panels/              one component per editor section; +page.svelte is just composition
  grid/                the module-grid editor overlay: GridEditor (the full-viewport
                       dialog), GridCanvas (pan/zoom board), GridModuleTile,
                       GridHoverCard, VaultDock, ModuleEditDialog — docs/grid-editor.md
...
`src/lib/game/ui-sounds.json` (~44 kB of MP3 as data URIs). The key is the **sfx name** the game
files a sound under — `UI/OK`, `UI/Step` — not the clip file name, because a clip name says nothing
and an audio pass can repoint a name at a different file. Eight are borrowed, all of them menu sounds:
...
| `open` / `close` | `UI/Grid/Open` / `Close` | a `Dialog` arrives and leaves (the grid editor overlay too — these are that screen's own sounds) — and `close` again under every edit: a tank unit, a wheel slot, a `+`/`-` step, a connection cell, a number field clicked, a consumable added or removed |
| `select` | `UI/Grid/ModuleSelected` | an effect-field shape is picked; a grid carry starts or is cancelled (the game points `startMoveSfx` *and* `cancelMoveSfx` at this one) |
| `place` | `UI/Grid/ModulePlaced` | the picker's **Add**, and a module landing on the grid |
| `placeCore` | `UI/Grid/PowerCorePlacement` | POWER CORE landing on the grid — the one module with its own `gridPlacementSfx`. `module-info.json`'s `placeSfx` carries each module's sfx *name* and `soundForSfx` maps it onto the palette, so an audio pass that gives more modules their own sound flows through on re-extraction |
| `fail` | `UI/Grid/PlacementFailed` | a drop the grid rules refuse |
