<script lang="ts">
	import icons from '$lib/save/resource-icons.json';
	import { iconStyle } from '$lib/save/pixel-icon';
	...
	// box. See the rule in $lib/save/pixel-icon.ts.
	let { id, scale = 2 }: { id: string; scale?: number } = $props();
